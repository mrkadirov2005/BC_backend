...
const asciiCodes = [
    ...
    const readUID=(num)=>{
        const splitted_storable=`${num}`.split("")
        // divide into 8 digits
        const scope=[]
        
        while(splitted_storable.length>=8){
            var spliced=splitted_storable.splice(0,8)
            scope.push(spliced)
        }
        
        // turn into ascii table version
        const ascii=[]
        for( let bin in scope){
            
            var inner_bin=0
            var index=0
            while(scope[bin].length>0){
                const popped_item=scope[bin].pop()
                var cal=popped_item*2**index
                index++
                inner_bin=cal+inner_bin
            }
            ascii.push(inner_bin)
    ...
    for(let i in ascii){
        for(let x in asciiCodes){
            if(ascii[i]==asciiCodes[x]["code"]){
                string.push(asciiCodes[x]["symbol"])
